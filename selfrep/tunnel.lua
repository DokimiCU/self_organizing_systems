...
minetest.register_abm{
     	nodenames = {"selfrep:selfrep_tunnel"},
	interval = 1,
	chance = 1,
	action = function(pos)
	...
	--count tunnels
		...
		local ps, cn = minetest.find_nodes_in_area(
			...
			{x = pos.x + radius, y = pos.y + radius, z = pos.z + radius}, {"selfrep:selfrep_tunnel"})
		num_tunnels = (cn["selfrep:selfrep_tunnel"] or 0)
...
if num_tunnels < growthlimittunnel then 
	...
	local xp_new = minetest.get_node(plusxpos)
		...
		if minetest.get_item_group(xp_new.name, "stone") == 1 then
			...
			--spread
			minetest.set_node(plusxpos, {name = "selfrep:selfrep_tunnel"})
	...
	--name for stone check
		...
		if minetest.get_item_group(xn_new.name, "stone") == 1 then
			...
			--spread
			minetest.set_node(negxpos, {name = "selfrep:selfrep_tunnel"})
	...
	--name for stone check
		...
		if minetest.get_item_group(yp_new.name, "stone") == 1 then
			...
			--spread
			minetest.set_node(plusypos, {name = "selfrep:selfrep_tunnel"})
	...
	--name for stone check
		...
		if minetest.get_item_group(zp_new.name, "stone") == 1 then
			...
			--spread
			minetest.set_node(pluszpos, {name = "selfrep:selfrep_tunnel"})
	...
	--expand z neg
		...
		if minetest.get_item_group(zn_new.name, "stone") == 1 then
			...
			--spread
			minetest.set_node(negzpos, {name = "selfrep:selfrep_tunnel"})
