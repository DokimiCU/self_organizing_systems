...
minetest.register_abm{
     	nodenames = {"selfrep_doomsday:selfrep_doomsday_dome"},
	interval = 1,
	chance = 1,
	action = function(pos)
...
negzpos = {x = pos.x, y = pos.y, z = pos.z - 1}
		...
		if minetest.get_node(plusxpos).name == "air" then
			--spread
			minetest.set_node(plusxpos, {name = "selfrep_doomsday:selfrep_doomsday_dome"})		
			--road build sound
		...
		if minetest.get_node(negxpos).name == "air" then
			--spread
			minetest.set_node(negxpos, {name = "selfrep_doomsday:selfrep_doomsday_dome"})	
			--road build sound
			minetest.sound_play("selfrep_roadbuild", {pos = pos, gain = 0.3, max_hear_distance = 15,})
			end
		...
		if minetest.get_node(plusypos).name == "air" then
			--spread
			minetest.set_node(plusypos, {name = "selfrep_doomsday:selfrep_doomsday_dome"})		
			--road build sound
			minetest.sound_play("selfrep_roadbuild", {pos = pos, gain = 0.3, max_hear_distance = 15,})
			end
		...
		if minetest.get_node(negypos).name == "air" then
			--spread
			minetest.set_node(negypos, {name = "selfrep_doomsday:selfrep_doomsday_dome"})	
			--road build sound
			minetest.sound_play("selfrep_roadbuild", {pos = pos, gain = 0.3, max_hear_distance = 15,})
			end
		...
		if minetest.get_node(pluszpos).name == "air" then
			--spread
			minetest.set_node(pluszpos, {name = "selfrep_doomsday:selfrep_doomsday_dome"})	
			--road build sound
			minetest.sound_play("selfrep_roadbuild", {pos = pos, gain = 0.3, max_hear_distance = 15,})
			end
		...
		if minetest.get_node(negzpos).name == "air" then
			--spread
			minetest.set_node(negzpos, {name = "selfrep_doomsday:selfrep_doomsday_dome"})		
			--road build sound
...
--is player near then space
		...
		for _,obj in ipairs(all_objects) do
			if obj:is_player() then
			minetest.set_node(pos, {name = "selfrep_doomsday:selfrep_doomsday_dome_gas"})
			end
		end
...
minetest.register_abm{
     	nodenames = {"selfrep_doomsday:selfrep_doomsday_dome_gas"},
	interval = 120,
	chance = 2,
	catch_up = false,
	action = function(pos)
